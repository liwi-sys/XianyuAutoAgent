...
class XianyuReplyBot:
    ...
    def generate_reply(self, user_msg: str, item_desc: str, context: List[Dict]) -> str:
        """生成回复主流程"""
        return self.generate_reply_with_model(user_msg, item_desc, context)
    
    def generate_reply_with_model(self, user_msg: str, item_desc: str, context: List[Dict], model_name: str = None) -> str:
        """使用指定模型生成回复主流程"""
        # 记录用户消息
        ...
        # 4. 生成回复（使用指定模型）
        return agent.generate_with_model(
            user_msg=user_msg,
            item_desc=item_desc,
...
class BaseAgent:
    ...
    def generate(
        ...
        """生成回复模板方法"""
        return self.generate_with_model(user_msg, item_desc, context, bargain_count)
    
    def generate_with_model(
        self, user_msg: str, item_desc: str, context: str, bargain_count: int = 0, model_name: str = None
    ) -> str:
        """使用指定模型生成回复模板方法"""
        messages = self._build_messages(user_msg, item_desc, context)
        response = self._call_llm_with_model(messages, model_name)
    ...
    def _call_llm(self, messages: List[Dict], temperature: float = 0.4) -> str:
        """调用大模型"""
        return self._call_llm_with_model(messages, temperature)
    
    def _call_llm_with_model(self, messages: List[Dict], temperature: float = 0.4, model_name: str = None) -> str:
        """使用指定模型调用大模型"""
        if model_name is None:
            if self.config_manager:
                model_name = self.config_manager.get("llm.default_model", "qwen-max")
            else:
                model_name = os.getenv("MODEL_NAME", "qwen-max")
        
        if self.config_manager:
            max_tokens = self.config_manager.get("llm.max_tokens", 500)
            top_p = self.config_manager.get("llm.top_p", 0.8)
        else:
            max_tokens = 500
            top_p = 0.8
...
class DefaultAgent(BaseAgent):
    ...
    def generate_with_model(
        ...
        """限制默认回复长度"""
        messages = self._build_messages(user_msg, item_desc, context)
        response = self._call_llm_with_model(messages, temperature=0.7, model_name=model_name)
        return response
